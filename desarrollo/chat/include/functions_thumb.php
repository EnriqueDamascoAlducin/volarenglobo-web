...
function create_thumbnail_watermark($targetPath, $targetFile, $sourceFile, $widthNew, $heightNew, $qualityNew)
{
...
// Logo check and var
    ...
    $szImgLogo = JAK_WATERMARK;

    if (stripos($szImgLogo, ".png") !== false) {
        $lpLogo = imagecreatefrompng($szImgLogo);
    } elseif (stripos($szImgLogo, ".jpg") !== false) {
        $lpLogo = imagecreatefromjpeg($szImgLogo);
    } elseif (stripos($szImgLogo, ".gif") !== false) {
        $lpLogo = imagecreatefromgif($szImgLogo);
    }
    ...
    $height_logo = imagesy($lpLogo);
        ...
        if (JAK_WMPOSITION == 1) {
            $j = 0;
            $i = 0;
        } elseif (JAK_WMPOSITION == 2) {
            $j = ($width - $width_logo) / 2;
            $i = 0;
        } elseif (JAK_WMPOSITION == 3) {
            $j = $width - $width_logo;
            $i = 0;
        } elseif (JAK_WMPOSITION == 4) {
            $j = 0;
            $i = ($height - $height_logo) / 2;
        } elseif (JAK_WMPOSITION == 5) {
            $j = ($width - $width_logo) / 2;
            $i = ($height - $height_logo) / 2;
        } elseif (JAK_WMPOSITION == 6) {
            $j = $width - $width_logo;
            $i = ($height - $height_logo) / 2;
        } elseif (JAK_WMPOSITION == 7) {
            $j = 0;
            $i = $height - $height_logo;
        } elseif (JAK_WMPOSITION == 8) {
            $j = ($width - $width_logo) / 2;
            $i = $height - $height_logo;
        } else {
            $j = $width - $width_logo;
