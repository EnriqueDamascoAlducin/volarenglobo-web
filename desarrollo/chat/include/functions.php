...
// Store the archive
function archive_conversation($convid, $name, $email) {
	
	global $lsdb;
	
	$lsdb->query('SELECT * FROM '.DB_PREFIX.'archive WHERE convid = "'.$convid.'"');
	
	if($lsdb->affected_rows == 0) {
	
		$result = $lsdb->query('SELECT * FROM '.DB_PREFIX.'transcript WHERE convid = "'.$convid.'" ORDER BY id ASC');
		
		while($row = $result->fetch_assoc()) {
			
			$lsdb->query('INSERT INTO '.DB_PREFIX.'archive SET 
			name = "'.smartsql($row['name']).'",
			message = "'.smartsql($row['message']).'",
			user = "'.smartsql($row['user']).'",
			convid = "'.$row['convid'].'",
			time = "'.$row['time'].'",
			class = "'.smartsql($row['class']).'"');
		}
		
		$lsdb->query('INSERT INTO '.DB_PREFIX.'leads SET
		name = "'.smartsql($name).'",
