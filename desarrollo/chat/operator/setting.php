...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!is_numeric($defaults['ls_client'])) { $errors['e4'] = $tl['error']['e15']; }
    
    if (!is_numeric($defaults['ls_admin'])) { $errors['e4'] = $tl['error']['e15']; }
    
    if (!is_numeric($defaults['ls_conf'])) { $errors['e5'] = $tl['error']['e15']; }
    ...
    $sql = 'UPDATE '.DB_PREFIX.'setting SET value = CASE varname
        ...
        WHEN "client_refresh" THEN "'.smartsql($defaults['ls_client']).'"
        WHEN "admin_refresh" THEN "'.smartsql($defaults['ls_admin']).'"
        WHEN "conv_refresh" THEN "'.smartsql($defaults['ls_conf']).'"
        WHEN "inactiv" THEN "'.smartsql($defaults['ls_inactiv']).'"
        WHEN "end_flush" THEN "'.smartsql($defaults['ls_flush']).'"
    ...
    END
		WHERE varname IN ("title","email","feedback","captcha","captchachat","smilies","sitehttps","lang","useravatwidth","useravatheight","dateformat","timeformat","timezoneserver","client_refresh","admin_refresh","conv_refresh","inactiv","end_flush","offline_message","login_message","welcome_message","leave_message","thankyou_message","feedback_message","thankyou_feedback")';
