...
$lstable1 = DB_PREFIX.'archive';

switch ($page1) {
	case 'delete':
		
		$result = $lsdb->query('SELECT transcript FROM '.$lstable.' WHERE id = "'.smartsql($page2).'"');
   		$row = $result->fetch_assoc();
   			
   		$lsdb->query('DELETE FROM '.$lstable1.' WHERE convid = "'.$row['transcript'].'"');
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		    ...
		    $lockuser = $defaults['ls_delete_leads'];
		        ...
		        for ($i = 0; $i < count($lockuser); $i++) {
		            ...
		            $result = $lsdb->query('SELECT transcript FROM '.$lstable.' WHERE id = "'.smartsql($locked).'"');
		            $row = $result->fetch_assoc();
